...
class DeduplicationService:
    ...
    def find_matching_sensor(
        self, device_id: str, new_sensor_data: Dict[str, Any], threshold: float = 0.90
    ) -> Optional[Any]:
        """
        ...
        if not self.sensor_manager:
            logger.warning(
                "[Dedup] No sensor_manager available, cannot check for matches"
            )
            return None
        ...
        try:
            ...
            logger.info(
                f"[Dedup] Found {len(existing_sensors)} existing sensors for device {device_id}"
            )
            ...
            new_rid = new_sensor_data.get("source", {}).get(
                "element_resource_id", ""
            ) or new_sensor_data.get("element_resource_id", "")
            new_screen = new_sensor_data.get("source", {}).get(
                "screen_activity", ""
            ) or new_sensor_data.get("screen_activity", "")
            logger.info(
                f"[Dedup] New sensor: resource_id={new_rid}, screen={new_screen}"
            )
            ...
            for existing_obj in existing_sensors:
                ...
                score, reasons = self._calculate_sensor_similarity(
                    new_sensor_data, existing
                )

                existing_name = existing.get("friendly_name", "Unknown")
                existing_rid = (
                    existing.get("source", {}).get("element_resource_id", "")
                    if existing.get("source")
                    else ""
                )
                logger.info(
                    f"[Dedup] Compared with '{existing_name}' (rid={existing_rid}): score={score:.2f}, reasons={[r.value for r in reasons]}"
                )
            ...
            if best_match:
                sensor_name = (
                    best_match.friendly_name
                    if hasattr(best_match, "friendly_name")
                    else "Unknown"
                )
                logger.info(
                    f"[Dedup] Auto-match found: {sensor_name} (score: {best_score:.2f}, reasons: {[r.value for r in best_reasons]})"
                )
                return best_match

            logger.info(
                f"[Dedup] No match found above threshold {threshold} (highest score: {highest_score_found:.2f})"
            )
            return None
        ...
        try:
            ...
            for existing_obj in existing_sensors:
                ...
                if score >= threshold:
                    ...
                    sensor_name = existing.get("name") or existing.get(
            ...
            if matches:
                logger.info(
                    f"[Dedup] Found {len(matches)} similar sensors for new sensor"
                )
    ...
    def _calculate_sensor_similarity(
        self, new_sensor: Dict[str, Any], existing: Dict[str, Any]
        ...
        if new_screen and existing_screen:
            ...
            new_activity = (
                new_screen.split(".")[-1] if "." in new_screen else new_screen
            )
            existing_activity = (
                existing_screen.split(".")[-1]
                if "." in existing_screen
                else existing_screen
            )
        ...
        new_bounds = new_sensor.get("source", {}).get(
            "custom_bounds"
        ) or new_sensor.get("bounds", {})
        existing_bounds = existing.get("source", {}).get(
            "custom_bounds"
        ) or existing.get("bounds", {})
        ...
        # Similar name (5%)
        new_name = (
            new_sensor.get("name", "") or new_sensor.get("friendly_name", "") or ""
        ).lower()
        existing_name = (
            existing.get("name", "") or existing.get("friendly_name", "") or ""
        ).lower()
        if new_name and existing_name:
            if (
                new_name == existing_name
                or new_name in existing_name
    ...
    def _bounds_overlap(
        self, bounds1: Dict, bounds2: Dict, tolerance: int = None
    ) -> bool:
    ...
    def find_similar_actions(
        ...
        try:
            ...
            for existing in existing_actions:
                ...
                if score >= threshold:
                    recommendation = self._get_recommendation(score)
                    action_name = existing.get("name") or existing.get(
                        "action", {}
                    ).get("name", "Unnamed Action")
                    action_type = existing.get("action_type") or existing.get(
                        "action", {}
    ...
    def _calculate_action_similarity(
        self, new_action: Dict[str, Any], existing: Dict[str, Any]
    ) -> Tuple[float, List[MatchReason]]:
        ...
        if new_type and existing_type and new_type == existing_type:
            ...
            if new_type == "tap":
                ...
                if (
                    ...
                    and ex_x is not None
                    and ex_y is not None
                ):
                    dx = abs(new_x - ex_x)
                    dy = abs(new_y - ex_y)
            ...
            # For swipe actions, check start/end coordinates (25%)
                ...
                if all(
                    ...
                    if (
                        d_start <= self.BOUNDS_TOLERANCE * 2
                        and d_end <= self.BOUNDS_TOLERANCE * 2
                    ):
                        score += 0.25
                        reasons.append(MatchReason.SAME_TARGET)
        ...
        if (
            new_name
            and ex_name
            and (new_name == ex_name or new_name in ex_name or ex_name in new_name)
        ):
            score += 0.05
            reasons.append(MatchReason.SIMILAR_NAME)
    ...
    def _calculate_flow_overlap(
        self, new_flow: Dict[str, Any], existing
    ) -> Tuple[float, List[MatchReason]]:
        ...
        if new_sensors and existing_sensors:
            ...
            if overlap:
                overlap_ratio = len(overlap) / max(
                    len(new_sensors), len(existing_sensors)
                )
        ...
        if new_screens and existing_screens:
            ...
            if overlap:
                overlap_ratio = len(overlap) / max(
                    len(new_screens), len(existing_screens)
                )
                score += overlap_ratio * 0.50
    ...
    def get_optimization_suggestions(self, device_id: str) -> Dict[str, Any]:
        ...
        suggestions = {
            ...
            "actions": [],
            "flows": [],
            "summary": {"total_duplicates": 0, "potential_savings": 0},
        }
        ...
        if self.sensor_manager:
            ...
            suggestions["summary"]["total_duplicates"] += sum(
                len(g["items"]) - 1 for g in sensor_groups
            )
        ...
        if self.action_manager:
            ...
            suggestions["summary"]["total_duplicates"] += sum(
                len(g["items"]) - 1 for g in action_groups
            )
    ...
    def _find_sensor_duplicate_groups(self, device_id: str) -> List[Dict]:
        ...
        try:
            ...
            for sensor in sensors:
                ...
                similar = self.find_similar_sensors(
                    device_id, sensor, threshold=self.MEDIUM_SIMILARITY
                )
                ...
                if similar_ids:
                    group = {
                        ...
                        "names": [sensor.get("name", "Unnamed")]
                        + [m.entity_name for m in similar],
    ...
    def _find_action_duplicate_groups(self, device_id: str) -> List[Dict]:
        ...
        try:
            ...
            for action in actions:
                ...
                similar = self.find_similar_actions(
                    device_id, action, threshold=self.MEDIUM_SIMILARITY
                )
    ...
    def _find_flow_overlaps(self, device_id: str) -> List[Dict]:
        ...
        try:
            ...
            for flow in flows:
                ...
                flow_dict = flow.dict()
                similar = self.find_overlapping_flows(
                    device_id, flow_dict, threshold=self.MEDIUM_SIMILARITY
                )
                similar_ids = [
                    m.entity_id for m in similar if m.entity_id != flow.flow_id
                ]
