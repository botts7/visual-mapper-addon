...
class ScreenshotStitcher:
    ...
    def __init__(self, adb_bridge):
        ...
        self.scroll_delay_ms = (
            1000  # Wait 1 second after scroll for animation to settle (was 600ms)
        )
        ...
        self.min_new_content_ratio = (
            0.05  # Need at least 5% new content to continue (was 15% - too strict)
        )
        self.fixed_element_threshold = (
            0.92  # Threshold for detecting fixed UI elements (allow slight variations)
        )
        ...
        if FEATURE_MATCHING_AVAILABLE:
            try:
                ...
                logger.warning(
                    f"[ScreenshotStitcher] Feature matching init failed: {e}"
                )
    ...
    async def estimate_content_height(self, device_id: str, screen_height: int) -> dict:
        ...
        try:
            ...
            if total_from_pattern and total_from_pattern.get("total_items"):
                ...
                avg_height = (
                    bounds_info.get("avg_item_height", 200) if bounds_info else 200
                )
                header_height = (
                    bounds_info.get("header_estimate", 500) if bounds_info else 500
                )
                final_estimate = header_height + (total_items * avg_height)
                confidence = "high"
                logger.info(
                    f"[HeightEstimate] Pattern match: {total_items} items * {avg_height}px + {header_height}px header = {final_estimate}px"
                )
                ...
                avg_height = max(
                    seq_height, bounds_height, 200
                )  # At least 200px per item
                ...
                if header_height <= 0:
                    ...
                    if bounds_info:
                        ...
                        header_height = max(
                            screen_minus_scroll + 600, 800
                        )  # Add buffer for content above list

                    logger.info(
                        f"[HeightEstimate] Scrolled view - using estimated header={header_height}px"
                    )
                ...
                logger.info(
                    f"[HeightEstimate] Footer: android_nav({android_nav}) + app_nav({app_nav}) = {footer_height}px"
                )

                final_estimate = (
                    header_height + (total_items * avg_height) + footer_height
                )
                confidence = "medium-high"
                logger.info(
                    f"[HeightEstimate] Numbered sequence: header({header_height}) + {total_items} items * {avg_height}px + footer({footer_height}) = {final_estimate}px"
                )
            ...
            return {
                ...
                "estimated_scrolls": max(
                    1, int((final_estimate - screen_height) / 400)
                ),
            ...
            traceback.print_exc()
            return {
                "estimated_height": screen_height,
                "confidence": "low",
                "error": str(e),
            }
    ...
    async def capture_scrolling_screenshot(
        ...
        try:
            ...
            is_browser = any(
                pkg in current_app.lower() for pkg in skip_refresh_packages
            )
            ...
            logger.info(
                f"  TOP: {len(elements_top)} UI elements, screen {width}x{height}"
            )
            ...
            logger.info(
                f"  OVERLAP CHECK: {len(overlap)} common elements between TOP and BOTTOM"
            )
            ...
            logger.info(
                f"  ELEMENT ANALYSIS: {moving_elements} moving, {fixed_elements} fixed"
            )
            ...
            if moving_elements >= 5:
                ...
                if is_short_page:
                    logger.info(
                        f"  SHORT PAGE: {moving_elements} moving + {pixel_diff:.1f}% diff"
                    )
            else:
                # Few/no moving elements = different content = LONG page
                is_short_page = False
                logger.info(
                    f"  LONG PAGE: only {moving_elements} moving elements (need middle captures)"
                )
            ...
            if is_short_page:
                ...
                overlap_end_y = self._find_overlap_end_y(
                    elements_top, elements_bottom, height
                )
                ...
                logger.info(
                    "  Strategy: Scroll from TOP to BOTTOM, capturing at each step"
                )
                ...
                scrollable_height = (
                    height - fixed_header - 100
                )  # Subtract header and some footer
                swipe_distance = int(
                    scrollable_height * 0.30
                )  # ~520px, gives more overlap
                ...
                captures = [
                    (img_top, elements_top, 0, 0)
                ]  # First capture: known_scroll=0
                ...
                for i in range(max_scrolls):
                    ...
                    logger.info(
                        f"  >>> SLOW SWIPE: y={swipe_start_y}->{swipe_end_y} ({swipe_distance}px, 1000ms)"
                    )
            ...
            stitched, combined_elements, stitch_info = self._stitch_by_elements(
                captures, height
            )
            ...
            metadata = {
                ...
                "bottom_reached": True,
                "avg_scroll_time_ms": (
                    duration_ms // max(1, scroll_count)
                    if scroll_count > 0
                    else duration_ms
                ),
            ...
            logger.info(
                f"[ScreenshotStitcher] Complete: {final_width}x{final_height} in {duration_ms}ms"
            )
            logger.info(
                f"  Strategy: {metadata['strategy']}, Scrolls: {scroll_count}, Captures: {len(captures)}"
            )
    ...
    def _find_overlap_end_y(
        self, elements_prev: list, elements_curr: list, height: int
    ) -> int:
        """Delegate to element analyzer."""
        return self.element_analyzer.find_overlap_end_y(
            elements_prev, elements_curr, height
        )
    ...
    def _calculate_scroll_from_elements(
        ...
        """Delegate to element analyzer."""
        return self.element_analyzer.calculate_scroll_from_elements(
            elements1, elements2, screen_height
        )
    ...
    def _find_new_content_boundary(
        ...
        """Delegate to element analyzer."""
        return self.element_analyzer.find_new_content_boundary(
            elements1, elements2, scroll_amount, screen_height
        )

    async def _get_ui_elements_with_retry(
        self, device_id: str, max_retries: int = 3
    ) -> list:
        """Delegate to device controller."""
        return await self.device_controller.get_ui_elements_with_retry(
            device_id, max_retries
        )
    ...
    def _stitch_by_elements(
        self, captures: list, screen_height: int
    ...
    def _remove_consecutive_duplicates(
        self, img: Image.Image, elements: list, screen_height: int
    ...
    def _detect_overlap_between_captures(
        ...
        """Delegate to overlap detector."""
        return self.overlap_detector.detect_overlap_between_captures(
            img1, img2, screen_height, known_scroll
        )
    ...
    def _calculate_scroll_offset(
        ...
        """Delegate to element analyzer."""
        return self.element_analyzer.calculate_scroll_offset(
            elements_prev, elements_curr, height
        )
    ...
    def _stitch_images_smart(
        ...
        """Delegate to image composer."""
        return self.image_composer.stitch_images_smart(
            captures, overlap_ratio, screen_height
        )

    def _stitch_images(self, images: list, overlap_ratio: float) -> Image.Image:
        """Delegate to image composer."""
