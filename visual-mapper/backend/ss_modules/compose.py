...
class ImageComposer:
    ...
    def __init__(self, overlap_detector, element_analyzer, remove_duplicates_fn, feature_stitcher=None):
        """
    ...
    def stitch_by_elements(
        ...
        for i in range(1, len(captures)):
            ...
            if precalc_first_new_y > 0:
                ...
                detected_footer = self.overlap_detector.detect_fixed_bottom_height(prev_raw_img, img_next)
                ...
                detected_new_content_start, detected_footer = self.overlap_detector.detect_overlap_between_captures(
    ...
    def stitch_two_captures_deterministic(
        ...
        fixed_header = self.overlap_detector.detect_fixed_top_height(img1, img2) if img1_height == screen_height else 0
        if fixed_header < 50:
    ...
    def stitch_two_captures(
        ...
        if img1_height > height:
            ...
            fixed_footer_height = self.overlap_detector.detect_fixed_bottom_height(img1_bottom, img2)
        else:
            fixed_footer_height = self.overlap_detector.detect_fixed_bottom_height(img1, img2)
        ...
        template_offset = self.overlap_detector.find_overlap_by_image(img1, img2, height)
    ...
    def stitch_images_smart(
        ...
        if len(captures) >= 2:
            fixed_bottom_height = self.overlap_detector.detect_fixed_bottom_height(images[0], images[1])
        ...
        for i, (img, elements) in enumerate(captures):
            if i == 0:
                ...
                scroll_amount, confidence = self.element_analyzer.calculate_scroll_from_elements(
                    prev_elements, elements, height
                )

                if scroll_amount and confidence > 0.3:
                    # Calculate crop_top based on element positions
                    crop_top = self.element_analyzer.find_new_content_boundary(prev_elements, elements, scroll_amount, height)
                else:
                    # Fallback to pixel-based
                    offset_y, _ = self.overlap_detector.find_overlap_offset(prev_img, img, overlap_height)
    ...
    def stitch_images(
        ...
        for i in range(1, len(images)):
            ...
            offset_y, _ = self.overlap_detector.find_overlap_offset(images[i-1], images[i], overlap_height)
