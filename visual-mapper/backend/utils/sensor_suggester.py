...
class SensorSuggester:
    ...
    PATTERNS = {
        ...
        'distance': {
            'keywords': ['distance', 'range', 'mileage', 'odometer', 'miles', 'kilometers', 'total mileage'],
        ...
        # =====================================================================
        'signal_strength': {
            'keywords': ['signal', 'wifi', 'bars', 'rssi', 'cellular', 'reception', 'network'],
        ...
        'data_size': {
            'keywords': ['storage', 'memory', 'disk', 'space', 'available', 'free', 'used'],
        ...
        'aqi': {
            'keywords': ['aqi', 'air quality', 'pm2.5', 'pm10', 'pollution', 'particulate'],
        ...
        # =====================================================================
        'monetary': {
            'keywords': ['price', 'cost', 'balance', 'amount', 'total', 'payment', 'fee'],
            'indicators': ['$', '€', '£', '¥', 'usd', 'eur'],
        ...
        'duration': {
            'keywords': ['remaining', 'elapsed', 'eta', 'time left', 'duration', 'countdown'],
        ...
        'timestamp': {
            'keywords': ['updated', 'last', 'time', 'date', 'modified', 'refreshed', 'synced', 'at'],
        ...
        'binary': {
            'keywords': ['status', 'state', 'enabled', 'disabled', 'active', 'mode', 'locked', 'door', 'window'],
            'indicators': [
                'on', 'off', 'true', 'false', 'yes', 'no', 'enabled', 'disabled',
                'active', 'inactive', 'online', 'offline', 'connected', 'disconnected',
                'locked', 'unlocked', 'open', 'closed', 'opened', 'armed', 'disarmed',
                'running', 'stopped', 'charging', 'ready', 'ok', 'error', 'good', 'bad',
                'home', 'away', 'present', 'absent', 'occupied', 'vacant'
            ],
            'device_class': 'none',
            'unit': None,
    ...
    def _fuzzy_match(self, text: str, keywords: List[str], threshold: float = 0.8) -> bool:
        """
        ...
        for keyword in keywords:
            ...
            for word in text_lower.split():
                ...
                if ratio >= threshold:
                    logger.debug(f"[SensorSuggester] Fuzzy match: '{keyword}' ~ '{word}' ({ratio:.2f})")
                    return True
        ...
        for other in elements:
            ...
            if self._looks_like_label(label_text):
                ...
                is_vertically_aligned = (
                    x_center_distance < 100 or
                    (other_bounds['x'] < bounds['x'] + element_width and
                     other_bounds['x'] + other_width > bounds['x'])
                )
