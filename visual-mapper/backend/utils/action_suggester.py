...
class ActionSuggester:
    ...
    def suggest_actions(self, elements: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        seen_combinations = (
            set()
        )  # Avoid duplicate suggestions (resource_id + text + bounds)
        ...
        for element in elements:
            ...
            analyzed += 1
            logger.debug(
                f"[ActionSuggester] Analyzing element: text='{text[:50] if text else '(none)'}', class='{element_class}', interactive={is_interactive}"
            )
            ...
            for pattern_name, pattern in self.PATTERNS.items():
                ...
                if match_result["matches"]:
                    ...
                    if unique_key in seen_combinations:
                        skipped_duplicate += 1
                        logger.debug(
                            f"[ActionSuggester] Skipping duplicate element: {unique_key}"
                        )
                        break
                    ...
                    matched_any = True
                    logger.debug(
                        f"[ActionSuggester] Matched pattern '{pattern_name}' with confidence {match_result['confidence']:.2f}"
                    )
            ...
            if not matched_any:
                logger.debug(
                    f"[ActionSuggester] No pattern matched for element: text='{text[:50] if text else '(none)'}', class='{element_class}'"
                )
        ...
        logger.info(
            f"[ActionSuggester] Generated {len(suggestions)} action suggestions from {len(elements)} elements"
        )
        logger.info(
            f"[ActionSuggester] Stats: analyzed={analyzed}, skipped_non_interactive={skipped_non_interactive}, skipped_duplicate={skipped_duplicate}, skipped_sensor_like={skipped_sensor_like}, skipped_wrapper={skipped_wrapper}"
        )
        return suggestions
    ...
    def _matches_pattern(
        ...
        # For generic patterns, accept clickable alone (must be truly clickable, not just focusable)
            ...
            pattern.get("is_generic")
            and clickable
            and not text_lower.startswith("android")
        ):
            confidence = pattern["confidence_base"] * 0.9
    ...
    def _is_wrapper_element(self, element: Dict[str, Any]) -> bool:
        ...
        if element_class in wrapper_classes:
            if not text and not content_desc:
                logger.debug(
                    f"[ActionSuggester] Skipping wrapper element: '{element_class}'"
                )
                return True
