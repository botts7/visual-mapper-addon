...
class DeviceMigrator:
    ...
    def __init__(self, data_dir: str = "data", config_dir: str = "config"):
        ...
        self.device_map: Dict[str, Set[str]] = {}  # stable_device_id -> set of device_ids
    ...
    def _load_device_mappings(self):
        ...
        logger.info(f"[DeviceMigrator] Loaded {len(self.device_map)} unique devices with {sum(len(ids) for ids in self.device_map.values())} total device IDs")
    ...
    def migrate_device_config(self, old_device_id: str, new_device_id: str, stable_device_id: str) -> Dict[str, int]:
        """
        Migrate all configurations from old device ID to new device ID.
        Returns counts of migrated items.
        """
        logger.info(f"[DeviceMigrator] Migrating device config from {old_device_id} to {new_device_id}")
        counts = {"sensors": 0, "actions": 0, "flows": 0}

        # 1. Migrate sensor definitions
        old_sensor_file = self.data_dir / f"sensors_{old_device_id.replace(':', '_').replace('.', '_')}.json"
        new_sensor_file = self.data_dir / f"sensors_{new_device_id.replace(':', '_').replace('.', '_')}.json"
        ...
        if old_sensor_file.exists() and not new_sensor_file.exists():
            try:
                ...
                logger.info(f"[DeviceMigrator] Migrated {counts['sensors']} sensors to {new_sensor_file}")
            except Exception as e:
                logger.error(f"[DeviceMigrator] Failed to migrate sensors: {e}")

        # 2. Migrate action definitions
        old_actions_dir = self.data_dir / f"actions_{old_device_id.replace(':', '_').replace('.', '_')}"
        new_actions_dir = self.data_dir / f"actions_{new_device_id.replace(':', '_').replace('.', '_')}"
            ...
            try:
                ...
                for action_file in new_actions_dir.glob("*.json"):
                    try:
                        ...
                        logger.error(f"[DeviceMigrator] Failed to update action file {action_file}: {e}")

                logger.info(f"[DeviceMigrator] Migrated {counts['actions']} actions to {new_actions_dir}")
            except Exception as e:
        ...
        if flows_dir.exists():
            flow_files = glob.glob(str(flows_dir / f"flow_{old_device_id.replace(':', '_').replace('.', '_')}_*.json"))
            for old_flow_file in flow_files:
                try:
                    ...
                    counts["flows"] += 1
                    logger.info(f"[DeviceMigrator] Migrated flow {old_flow_id} to {new_flow_id}")
                except Exception as e:
                    logger.error(f"[DeviceMigrator] Failed to migrate flow {old_flow_file}: {e}")
        ...
        logger.info(f"[DeviceMigrator] Migration complete: {counts['sensors']} sensors, {counts['actions']} actions, {counts['flows']} flows")
        return counts

    def check_and_migrate(self, new_device_id: str, stable_device_id: str) -> Optional[Dict[str, int]]:
        """
        ...
        if not self.is_known_device(stable_device_id):
            logger.debug(f"[DeviceMigrator] Device {new_device_id} ({stable_device_id}) is new - no migration needed")
            return None

        # Check if this exact device_id already exists
        if new_device_id in self.device_map.get(stable_device_id, set()):
            logger.debug(f"[DeviceMigrator] Device {new_device_id} already exists - no migration needed")
            return None
        ...
        logger.info(f"[DeviceMigrator] Detected device address change: {old_device_id} -> {new_device_id}")
        logger.info(f"[DeviceMigrator] Stable device ID: {stable_device_id}")

        # Perform migration
        counts = self.migrate_device_config(old_device_id, new_device_id, stable_device_id)
        return counts
