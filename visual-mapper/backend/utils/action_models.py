...
class SwipeAction(ActionBase):
    ...
    duration: int = Field(default=300, ge=50, le=5000, description="Swipe duration in ms")
...
class KeyEventAction(ActionBase):
    ...
    keycode: str = Field(..., description="Android keycode (e.g., KEYCODE_HOME, KEYCODE_BACK)")
...
class LaunchAppAction(ActionBase):
    ...
    activity: Optional[str] = Field(None, description="Optional activity name to launch")

    @validator('package_name')
    def validate_package(cls, v):
        # Basic package name validation (should contain at least one dot)
        if '.' not in v:
            raise ValueError("Package name must contain at least one dot (e.g., com.example.app)")
...
class DelayAction(ActionBase):
    ...
    duration: int = Field(..., ge=10, le=60000, description="Delay duration in milliseconds")
...
class MacroAction(ActionBase):
    ...
    stop_on_error: bool = Field(default=False, description="Stop macro if any action fails")
...
ActionType = Annotated[
    Union[TapAction, SwipeAction, TextInputAction, KeyEventAction, LaunchAppAction, DelayAction, MacroAction],
...
class ActionExecutionRequest(BaseModel):
    ...
    action_id: Optional[str] = Field(None, description="Action ID to execute (if saved)")
    action: Optional[ActionType] = Field(None, description="Inline action to execute (if not saved)")
