...
class PlayStoreIconScraper:
    ...
    def get_icon(self, package_name: str) -> Optional[bytes]:
        ...
        try:
            ...
            if icon_data:
                ...
                logger.info(
                    f"[PlayStoreIconScraper] ✅ Scraped and cached icon for {package_name} ({len(icon_data)} bytes)"
                )
            ...
            logger.warning(
                f"[PlayStoreIconScraper] ❌ Failed to scrape icon for {package_name}: {e}"
            )
    ...
    def _scrape_icon_and_name(
        ...
        try:
            ...
            if not details:
                logger.warning(
                    f"[PlayStoreIconScraper] No details found for {package_name}"
                )
                return None, None
            ...
            if not icon_url:
                logger.warning(
                    f"[PlayStoreIconScraper] No icon URL found for {package_name}"
                )
            ...
            logger.error(
                f"[PlayStoreIconScraper] Failed to scrape icon/name for {package_name}: {e}"
            )
            return None, None

    def get_app_name(
        self, package_name: str, cache_only: bool = False
    ) -> Optional[str]:
        """
        ...
        if cached_name:
            logger.debug(
                f"[PlayStoreIconScraper] Name cache hit for {package_name}: {cached_name}"
            )
            return cached_name
        ...
        try:
            ...
            if app_name:
                ...
                self._cache_app_name(package_name, app_name)
                logger.info(
                    f"[PlayStoreIconScraper] ✅ Scraped app name for {package_name}: {app_name}"
                )
                return app_name
        except Exception as e:
            logger.warning(
                f"[PlayStoreIconScraper] ❌ Failed to scrape app name for {package_name}: {e}"
            )
    ...
    def _get_cached_app_name(self, package_name: str) -> Optional[str]:
        ...
        if metadata_path.exists():
            try:
                ...
                logger.debug(
                    f"[PlayStoreIconScraper] Failed to read metadata cache: {e}"
                )
        return None
    ...
    def _cache_app_name(self, package_name: str, app_name: str):
        ...
        metadata_path = self.metadata_dir / f"{package_name}.json"
        try:
            metadata = {"title": app_name, "package": package_name}
            with open(metadata_path, "w", encoding="utf-8") as f:
                json.dump(metadata, f, ensure_ascii=False, indent=2)
            logger.debug(
                f"[PlayStoreIconScraper] Cached app name: {package_name} -> {app_name}"
            )
        except Exception as e:
            logger.warning(f"[PlayStoreIconScraper] Failed to cache app name: {e}")

    def batch_scrape(
        self, package_names: list[str], max_apps: Optional[int] = None
    ) -> int:
        """
        ...
        for i, package_name in enumerate(
            package_names[:max_apps] if max_apps else package_names
        ):
            try:
                ...
                if icon_data:
                    success_count += 1
                    logger.info(
                        f"[PlayStoreIconScraper] {i+1}/{total}: ✅ {package_name}"
                    )
                else:
                    logger.warning(
                        f"[PlayStoreIconScraper] {i+1}/{total}: ❌ {package_name} (not found)"
                    )
            except Exception as e:
                logger.error(
                    f"[PlayStoreIconScraper] {i+1}/{total}: ❌ {package_name} ({e})"
                )

        logger.info(
            f"[PlayStoreIconScraper] Batch complete: {success_count}/{total} icons scraped"
        )
