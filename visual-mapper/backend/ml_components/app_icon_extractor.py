...
class AppIconExtractor:
    ...
    def __init__(
        self, cache_dir: str = "data/app-icons", enable_extraction: bool = True
    ):
        """
        ...
        logger.info(
            f"[AppIconExtractor] Initialized (cache: {cache_dir}, enabled: {enable_extraction})"
        )
    ...
    def get_icon(self, device_id: str, package_name: str) -> Optional[bytes]:
        ...
        if not self.enable_extraction:
            logger.debug(
                f"[AppIconExtractor] Extraction disabled, returning None for {package_name}"
            )
            return None
        ...
        try:
            ...
            if icon_data:
                ...
                logger.info(
                    f"[AppIconExtractor] Extracted and cached icon for {package_name} ({len(icon_data)} bytes)"
                )
                return icon_data
        except Exception as e:
            logger.warning(
                f"[AppIconExtractor] Failed to extract icon for {package_name}: {e}"
            )
    ...
    def _extract_icon_from_device(
        ...
        try:
            ...
            if not apk_path:
                logger.warning(
                    f"[AppIconExtractor] Could not find APK path for {package_name}"
                )
                return None
            ...
            logger.error(
                f"[AppIconExtractor] Extraction failed for {package_name}: {e}"
            )
            return None
    ...
    def _extract_icon_from_apk(
        self, apk_path: str, package_name: str
    ) -> Optional[bytes]:
        """
                ...
                for icon_name in icon_names:
                    for ext in extensions:
                        ...
                        if density != "anydpi-v26":
                            icon_paths.append(
                                f"res/{directory}-{density}-v4/{icon_name}{ext}"
                            )
        ...
        try:
            with zipfile.ZipFile(apk_path, "r") as apk_zip:
                ...
                logger.debug(
                    f"[AppIconExtractor] Searching {len(icon_paths)} icon paths for {package_name}"
                )
                ...
                logger.debug(
                    f"[AppIconExtractor] Specific paths failed, checking for adaptive icon XML..."
                )
                adaptive_icon_data = self._extract_adaptive_icon(
                    apk_zip, all_files, package_name
                )
                if adaptive_icon_data:
                    logger.info(
                        f"[AppIconExtractor] ✅ Extracted adaptive icon foreground for {package_name}"
                    )
                    return adaptive_icon_data

                # Fallback 2: Search for any ic_launcher variant (PNG or WebP)
                logger.debug(
                    f"[AppIconExtractor] Adaptive icon failed, trying pattern search..."
                )
                for file_path in all_files:
                    if "ic_launcher" in file_path and (
                        ...
                        logger.info(
                            f"[AppIconExtractor] ✅ Found icon via pattern search: {file_path}"
                        )
                        return apk_zip.read(file_path)

                # Fallback 3: Search for any file named 'icon' in mipmap/drawable
                logger.debug(
                    f"[AppIconExtractor] Pattern search failed, trying generic icon search..."
                )
                for file_path in all_files:
                    if (
                        ("mipmap" in file_path or "drawable" in file_path)
                        and "icon" in file_path.lower()
                        and (file_path.endswith(".png") or file_path.endswith(".webp"))
                    ):
                        logger.info(
                            f"[AppIconExtractor] ✅ Found icon via generic search: {file_path}"
                        )
                        return apk_zip.read(file_path)

                # Log which files ARE in the APK to help debug
                icon_related_files = [
                    f for f in all_files if "launcher" in f or "icon" in f.lower()
                ]
                if icon_related_files:
                    logger.warning(
                        f"[AppIconExtractor] ❌ No extractable icon found for {package_name}. Icon-related files: {icon_related_files[:10]}"
                    )
                else:
                    logger.warning(
                        f"[AppIconExtractor] ❌ No icon-related files found in APK for {package_name}"
                    )
    ...
    def _extract_adaptive_icon(
        self, apk_zip: zipfile.ZipFile, all_files: list, package_name: str
    ) -> Optional[bytes]:
        """
        ...
        try:
            ...
            for xml_path in adaptive_xml_paths:
                if xml_path in all_files:
                    logger.debug(
                        f"[AppIconExtractor] Found adaptive icon XML: {xml_path}"
                    )

                    # Read and parse XML
                    xml_content = apk_zip.read(xml_path).decode(
                        "utf-8", errors="ignore"
                    )
                    ...
                    foreground_match = re.search(
                        r'<foreground[^>]*android:drawable="@([^"]+)"', xml_content
                    )

                    if foreground_match:
                        drawable_ref = foreground_match.group(
                            1
                        )  # e.g., "drawable/ic_launcher_foreground"
                        logger.debug(
                            f"[AppIconExtractor] Adaptive icon foreground: {drawable_ref}"
                        )

                        # Try to find the actual drawable file (PNG/WebP)
                        # Search in different densities
                        for density in ["xxxhdpi", "xxhdpi", "xhdpi", "hdpi", "mdpi"]:
                            drawable_name = drawable_ref.split("/")[
                                -1
                            ]  # Get just the filename
                            drawable_dir = (
                                drawable_ref.split("/")[0]
                                if "/" in drawable_ref
                                else "drawable"
                            )
                            ...
                            for ext in [".png", ".webp"]:
                                possible_path = (
                                    f"res/{drawable_dir}-{density}/{drawable_name}{ext}"
                                )
                                if possible_path in all_files:
                                    logger.info(
                                        f"[AppIconExtractor] ✅ Found adaptive foreground at {possible_path}"
                                    )
