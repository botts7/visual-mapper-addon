...
class AppNameBackgroundFetcher:
    ...
    def request_name(self, package_name: str):
        ...
        if package_name in self.completed or package_name in self.failed:
            logger.debug(f"[AppNameBackgroundFetcher] Already processed: {package_name}")
            return

        # Skip if already in queue or processing
        if package_name in self.processing:
            logger.debug(f"[AppNameBackgroundFetcher] Already processing: {package_name}")
            return
        ...
        logger.debug(f"[AppNameBackgroundFetcher] Queued: {package_name} (queue size: {len(self.queue)})")
    ...
    async def _fetch_name(self, package_name: str) -> bool:
        ...
        try:
            ...
            if app_name:
                ...
                logger.warning(f"[AppNameBackgroundFetcher] ❌ No name found: {package_name}")
                return False

        except Exception as e:
            logger.error(f"[AppNameBackgroundFetcher] Fetch failed for {package_name}: {e}")
            return False

    async def prefetch_all_apps(self, packages: list[str], max_apps: Optional[int] = None):
        """
        ...
        """
        logger.info(f"[AppNameBackgroundFetcher] Prefetching names for {len(packages)} apps")
        ...
        logger.info(f"[AppNameBackgroundFetcher] Queued {len(packages_to_fetch)} apps for prefetch")
    ...
    def get_queue_stats(self) -> dict:
        ...
        if self.total_requested > 0:
            progress_percentage = round((total_processed / self.total_requested) * 100, 1)
